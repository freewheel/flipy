...
class GurobiSolver:
    ...
    def add_variables(self, lp_variables, model: gurobipy.Model) -> None:
        ...
        vars = model.addVars(
            len(lp_variables),
            lb=[var.low_bound for var in lp_variables],
            ub=[var.up_bound for var in lp_variables],
            obj=[var.obj_coeff for var in lp_variables],
            vtype=[gurobipy.GRB.CONTINUOUS if var.var_type == VarType.Continuous else gurobipy.GRB.INTEGER for var in lp_variables],
            name=[var.name for var in lp_variables]
        )
        model.update()
